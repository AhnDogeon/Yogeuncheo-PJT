...
import pandas as pd
import sys
sys.stdin = open('../../data_gdr/1-2. 서울 법정동 주소목록(최종).txt', 'r', encoding='UTF-8')



brand_list = []
for j in range(494):
    brand_list.append(str(input())+' 공원')
...
for brand in brand_list:
    if brand == '법정동명 공원' or brand == '서울특별시 공원' or len(brand.split(' ')) == 3:
        continue
        ...
        for _ in range(many_time):
            ...
            if(rescode==200):
                ...
                for j in range(len(py_rt['items'])):
                    # print(py_rt['items'][j])
                    if '서울특별시' in py_rt['items'][j]['address'] and real_gu in py_rt['items'][j]['address'] \
                            and dong in py_rt['items'][j]['address'] and code in py_rt['items'][j]['category'] and '부속시설' not in py_rt['items'][j]['category']:
                        # print(py_rt['items'][j])
