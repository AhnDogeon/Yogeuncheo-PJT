...
#     result = pd.read_csv('../data/data_adg/최종 결과.txt', sep=",", engine='python')
#     for i in range(len(result.index)):
#         data_id = result.loc[i][0]
#         data = TotalAddress.objects.get(id=data_id)
#         data.address = result.loc[i][1]
...
#         data.megabox_score = float(result.loc[i][12])
#         data.cgv_score = float(result.loc[i][13])
#         data.total_score = float(result.loc[i][13])
#
#         data.high_oil = float(result.loc[i][14])
#         data.oil = float(result.loc[i][15])
#         data.oil2 = float(result.loc[i][16])
#         data.total = int(result.loc[i][17])
#         data.catch = int(result.loc[i][18])
#         data.percent = float(result.loc[i][19])
#
#         data.mac_count = int(result.loc[i][20])
#         data.lot_count = int(result.loc[i][21])
#         data.burgerking_count = int(result.loc[i][22])
#         data.seveneleven_count = int(result.loc[i][23])
#         data.cu_count = int(result.loc[i][24])
#         data.gs25_count = int(result.loc[i][25])
#         data.starbucks_count = int(result.loc[i][26])
#         data.lottecinema_count = int(result.loc[i][27])
#         data.megabox_count = int(result.loc[i][28])
#         data.cgv_count = int(result.loc[i][29])
#         data.total = int(result.loc[i][30])
#
#         # create_data(address=address, gu=gu, dong=dong, mac_score=mac_score, lot_score=lot_score,burgerking_score=burgerking_score,
#         #             seveneleven_score=seveneleven_score, cu_score=cu_score, gs25_score=gs25_score, starbucks_score=starbucks_score)
#         print(i, '/', len(result))
#         data.save()
#     else:
#         print("create data done")
